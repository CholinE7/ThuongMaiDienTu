...
export default function AdminOrdersPage() {
  ...
  const calculateRevenue = () => {
    let total = 0, success = 0, pending = 0, cancelled = 0;
    orders.forEach(order => {
      const money = order.totalAmount || 0;
      total += money;
      if (order.status === "delivered") success += money;
      else if (order.status === "pending") pending += money;
      else if (order.status === "cancelled") cancelled += money;
    });
    return { total, success, pending, cancelled };
  };
  ...
  // Lọc theo tab ở phía client
  const filteredByTab = activeTab === "all" ? orders : orders.filter(o => o.status === activeTab);

  const totalPages = Math.ceil(filteredByTab.length / ORDERS_PER_PAGE);
  const startIndex = (currentPage - 1) * ORDERS_PER_PAGE;
  const paginatedOrders = filteredByTab.slice(startIndex, startIndex + ORDERS_PER_PAGE);
    ...
    <div className="relative min-h-[80vh] font-sans pb-10">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
        <div className="bg-gradient-to-r from-blue-600 to-blue-800 p-6 rounded-xl shadow-md text-white flex items-center gap-5">
          ...
          <div>
            <p className="text-blue-100 text-sm font-medium">Tổng tất cả đơn</p>
            <h3 className="text-2xl font-bold">{formatPrice(revenue.total)}</h3>
          </div>
        </div>
        <div className="bg-gradient-to-r from-green-600 to-green-800 p-6 rounded-xl shadow-md text-white flex items-center gap-5">
          <div className="text-3xl">✅</div>
          <div>
            <p className="text-green-100 text-sm font-medium">Đã thanh toán</p>
            <h3 className="text-2xl font-bold">{formatPrice(revenue.success)}</h3>
          </div>
        </div>
        <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200 flex items-center gap-5">
          <div className="text-3xl text-orange-500">⏳</div>
          <div>
            <p className="text-gray-500 text-sm font-semibold">Chờ thanh toán</p>
            <h3 className="text-xl font-bold text-gray-900">{formatPrice(revenue.pending)}</h3>
          </div>
        </div>
        <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200 flex items-center gap-5">
          <div className="text-3xl text-red-500">❌</div>
          <div>
            <p className="text-gray-500 text-sm font-semibold">Đơn đã hủy</p>
            <h3 className="text-xl font-bold text-gray-900">{formatPrice(revenue.cancelled)}</h3>
          </div>
        </div>
      </div>

      {/* TABS + BẢNG gộp trong 1 card */}
        ...
        <div className="flex border-b border-gray-200">
          {TABS.map((tab) => (
            <button
              ...
              className={`flex items-center gap-2 px-6 py-4 text-sm font-bold transition border-b-2 -mb-px whitespace-nowrap
                ${activeTab === tab.key
            ...
            >
              ...
              <span className={`text-xs px-2 py-0.5 rounded-full font-bold
                ${tab.key === "all" ? "bg-blue-100 text-blue-700" :
                  tab.key === "pending" ? "bg-orange-100 text-orange-600" :
                    tab.key === "delivered" ? "bg-green-100 text-green-700" :
