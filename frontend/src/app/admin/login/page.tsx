...
const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok && token !== "fail") {
        ...
        const checkRole = await fetch('http://localhost:8080/api/employers?page_no=0&page_size=1', {
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col justify-center py-12 px-4 sm:px-6 lg:px-8 font-sans">
      {/* TOAST NOTIFICATION */}
      {toastMsg && (
        <div className={`fixed top-6 right-6 px-6 py-4 rounded-xl shadow-2xl flex items-center gap-3 z-[60] animate-in slide-in-from-right-8 ${toastType === 'success' ? 'bg-green-600 text-white' : 'bg-red-600 text-white'}`}>
          {toastType === 'success' ? <CheckCircle2 size={24} /> : <XCircle size={24} />}
          <span className="font-bold tracking-wide uppercase text-sm">{toastMsg}</span>
        </div>
      )}
